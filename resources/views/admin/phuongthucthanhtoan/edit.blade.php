...
@section('content')
    <h2>Sửa phương thức thanh toán: {{ $phuongThucThanhToan->ten }}</h2>
    ...
    <form action="{{ route('admin.phuongthucthanhtoan.update', $phuongThucThanhToan->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <textarea name="mo_ta" id="mo_ta" class="form-control">{{ old('mo_ta', $phuongThucThanhToan->mo_ta) }}</textarea>
            @error('mo_ta')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <select name="hoat_dong" id="hoat_dong" class="form-control">
                <option value="1" {{ old('hoat_dong', $phuongThucThanhToan->hoat_dong) ? 'selected' : '' }}>Có</option>
                <option value="0" {{ !old('hoat_dong', $phuongThucThanhToan->hoat_dong) ? 'selected' : '' }}>Không</option>
            </select>
            @error('hoat_dong')
                <div class="text-danger">{{ $message }}</div>
            @enderror
