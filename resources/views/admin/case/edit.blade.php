...
@section('title', 'Sửa Cases')

@section('content')
    <div class="container">
        <h2 class="mb-4">Sửa Cases: {{ $cases->ten }}</h2>
        ...
        <form action="{{ route('admin.case.update', $cases->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="number" name="gia" id="gia" class="form-control" value="{{ old('gia',$cases->gia ?? '') }}">
                @error('gia')
            ...
            <div class="mb-3">
                ...
                <input type="number" name="gia_sale" id="gia_sale" class="form-control" value="{{ old('gia_sale',$cases->gia_sale ?? '') }}">
                @error('gia_sale')
                    <div class="text-danger small">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea name="mo_ta" id="mo_ta" class="form-control" rows="4">{{ old('mo_ta', $cases->mo_ta) }}</textarea>
                @error('mo_ta')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <a href="{{ route('admin.case.index') }}" class="btn btn-secondary">Hủy</a>
        </form>
    </div>
@endsection
