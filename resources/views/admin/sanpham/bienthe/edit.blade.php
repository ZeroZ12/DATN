...
@section('content')
    <div class="container">
        <h1>Chỉnh sửa biến thể sản phẩm: {{ $bienthe->sanpham->ten }}</h1>
        ...
        <form action="{{ route('admin.bienthe.update', $bienthe->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <input type="hidden" name="id_product" value="{{ $bienthe->sanpham->id }}">
            ...
            <div class="form-group">
                ...
                <select name="id_ram" id="id_ram" class="form-control @error('id_ram') is-invalid @enderror">
                    ...
                    @foreach ($ram as $r)
                        <option value="{{ $r->id }}" {{ old('id_ram', $bienthe->id_ram) == $r->id ? 'selected' : '' }}>
                            {{ $r->dung_luong }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select name="id_o_cung" id="id_o_cung" class="form-control @error('id_o_cung') is-invalid @enderror">
                    ...
                    @foreach ($ocung as $o)
                        <option value="{{ $o->id }}" {{ old('id_o_cung', $bienthe->id_o_cung) == $o->id ? 'selected' : '' }}>
                            {{ $o->loai }} - {{ $o->dung_luong }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input type="text" name="ma_bien_the" id="ma_bien_the"
                    class="form-control @error('ma_bien_the') is-invalid @enderror" value="{{ old('ma_bien_the', $bienthe->ma_bien_the) }}">
                @error('ma_bien_the')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="number" name="gia_so_sanh" id="gia_so_sanh"
                    class="form-control @error('gia_so_sanh') is-invalid @enderror" value="{{ old('gia_so_sanh', $bienthe->gia_so_sanh) }}">
                @error('gia_so_sanh')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="number" name="ton_kho" id="ton_kho"
                    class="form-control @error('ton_kho') is-invalid @enderror" value="{{ old('ton_kho', $bienthe->ton_kho) }}">
                @error('ton_kho')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                @if ($bienthe->anh_dai_dien)
                    <div class="mt-2">
                        <img src="{{ asset('storage/images/' . $bienthe->anh_dai_dien) }}" alt="Ảnh đại diện" width="100">
                    </div>
                @endif
            </div>
            ...
            <a href="{{ route('admin.bienthe.index', $bienthe->sanpham->id) }}" class="btn btn-secondary">Quay lại</a>
        </form>
    </div>
@endsection
