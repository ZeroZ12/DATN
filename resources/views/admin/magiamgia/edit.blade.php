...
@section('content')
    <h2>Sửa mã giảm giá: {{ $maGiamGia->ma }}</h2>
    ...
    <form action="{{ route('admin.magiamgia.update', $maGiamGia->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="loai" id="loai" class="form-control">
                <option value="phan_tram" {{ old('loai', $maGiamGia->loai) == 'phan_tram' ? 'selected' : '' }}>Phần trăm</option>
                <option value="tien_mat" {{ old('loai', $maGiamGia->loai) == 'tien_mat' ? 'selected' : '' }}>Tiền mặt</option>
            </select>
            @error('loai')
                <div class="text-danger">{{ $message }}</div>
            @enderror
