...
@section('title', 'Thùng rác chip')

@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h2 class="mb-0">Thùng rác - Chip đã xóa</h2>
        ...
        <tbody>
            @forelse($chips as $chip)
                <tr>
                    <td>{{ $chip->id }}</td>
                    <td>{{ $chip->ten }}</td>
                    <td>{{ $chip->deleted_at->format('d/m/Y H:i') }}</td>
                    <td>
                        <form action="{{ route('admin.chip.restore', $chip->id) }}" method="POST" class="d-inline">
                            @csrf
                            @method('PATCH')
                            <button class="btn btn-sm btn-success">Khôi phục</button>
                        </form>
                        <form action="{{ route('admin.chip.forceDelete', $chip->id) }}" method="POST" class="d-inline"
