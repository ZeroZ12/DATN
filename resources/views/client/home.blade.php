...
@section('content')
            ...
            <div class="product-section mb-4">
                ...
                <div class="products-slider-wrapper">
                    ...
                    <div class="products-slider">
                        @foreach ($sanphams->where('id_category', $danhMuc->id)->take(5) as $sp)
                            @php
                                if ($sp->co_bien_the) {
                                    $bienThe =
                            ...
                            <div class="product-card-gearvn position-relative">
                                <div class="product-image-gearvn">
                                    ...
                                    @elseif($gia_so_sanh && $gia_so_sanh > $gia)
                                        <span
                                            class="badge badge-sale">-{{ round((100 * ($gia_so_sanh - $gia)) / $gia_so_sanh) }}%</span>
                                    @endif
                                </div>
                                <div class="product-info-gearvn position-relative">
                                    <div class="product-title-gearvn">
                                        <a href="{{ route('sanpham.show', $sp->id) }}?variant={{ $bienThe->id ?? '' }}"
                                            class="text-decoration-none text-dark">{{ $sp->ten }}</a>
                                    </div>
                                    <div class="product-specs-gearvn">
                                        @if ($sp->chip)
                                            <span class="spec-tag">{{ $sp->chip->ten }}</span>
                                        @endif
                                        @if ($sp->gpu)
                                            <span class="spec-tag">{{ $sp->gpu->ten }}</span>
                                        @endif
                                        @if ($sp->mainboard)
                                            <span class="spec-tag">{{ $sp->mainboard->ten }}</span>
                                        @endif
                                        @if ($bienThe && $bienThe->ram)
                                            <span class="spec-tag">{{ $bienThe->ram->dung_luong }}</span>
                                        @endif
                                        @if ($bienThe && $bienThe->oCung)
                                            <span class="spec-tag">{{ $bienThe->oCung->dung_luong }}</span>
                                        @endif
                                    </div>
                                    <div class="product-price-gearvn">
                                        @if ($gia_so_sanh && $gia_so_sanh > $gia)
                                            <span class="old-price">{{ number_format($gia_so_sanh) }}₫</span>
                                        @endif
                                        <span class="current-price">{{ number_format($gia) }}₫</span>
                                    </div>
                                    <div class="product-rating-gearvn">
                                        @php
                                    ...
                                    <form action="{{ route('client.cart.add') }}" method="POST"
                                        ...
                                        <button type="submit"
                                            class="add-to-cart-btn-gearvn-rect btn btn-success w-100 d-flex align-items-center justify-content-center gap-2"
    ...
    @push('css')
        <style>
            ...
            .product-card-gearvn:hover {
                border: 1.5px solid #007bff;
                box-shadow: 0 4px 16px rgba(0, 123, 255, 0.08);
            }

            .product-image-gearvn {
                width: 100%;
                height: 180px;
                background: #f8f9fa;
                display: flex;
            ...
            .product-title-gearvn a {
                color: #222;
                text-decoration: none;
            }

            .product-title-gearvn a:hover {
                color: #007bff;
                text-decoration: underline;
            }

            .product-specs-gearvn {
                margin-bottom: 8px;
                display: flex;
                flex-wrap: wrap;
                gap: 4px;
            ...
            .product-price-gearvn .old-price {
                color: #b0b0b0;
                text-decoration: line-through;
                font-size: 13px;
            }

            .product-price-gearvn .current-price {
                color: #e53935;
                font-size: 18px;
                font-weight: 700;
            }

            .product-rating-gearvn {
                font-size: 13px;
                color: #888;
                display: flex;
                align-items: center;
            ...
            .add-to-cart-form.position-absolute {
                right: 10px;
                bottom: 10px;
            }

            .add-to-cart-btn-gearvn {
                background: #fff;
                color: #007bff;
            ...
            .add-to-cart-btn-gearvn:hover:not(:disabled) {
                background: #e3f2fd;
                color: #0056b3;
                border: 1.5px solid #007bff;
            }

            .add-to-cart-btn-gearvn:disabled {
                background: #f0f0f0;
                color: #aaa;
                border: 1px solid #e0e0e0;
                cursor: not-allowed;
            ...
            .add-to-cart-btn-gearvn-rect:hover:not(:disabled) {
                background: #218838;
                color: #fff;
                box-shadow: 0 4px 16px rgba(40, 167, 69, 0.12);
            }

            .add-to-cart-btn-gearvn-rect:disabled {
                background: #f0f0f0;
                color: #aaa;
                cursor: not-allowed;
            }
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                document.querySelectorAll('.add-to-cart-form').forEach(form => {
                    form.addEventListener('submit', function(event) {
                        console.log(
                            'Form submission detected for .add-to-cart-form. Preventing default action.'
                        );
